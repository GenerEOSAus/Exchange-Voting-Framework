...
const ProxyContextProvider = (props) => {
    ...
    async function loadProxy() {
        const res = await fetch('//localhost:3000/api/proxies');
        res.json()
            .then((data) => {
                data = data.map((item, idx) => { return {id: (idx+1), ...item}; }); //add index to each row
                setProxyList(data)
            })
    ...
    useEffect(() => {
        console.log("@@ CALLING Get Proxy List API");
        loadProxy();
